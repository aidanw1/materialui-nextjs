...
export default function Estimate() {
  ...
  const sendEstimate = () => {
    ...
    axios
      .get("https://us-central1-material-ui-course-951d5.cloudfunctions.net/sendMail", {
        params: {
          name: name,
          email: email,
          phone: phone,
        ...
        <DialogContent>
          <Grid container justify="space-around" direction={matchesSM ? "column" : "row"} alignItems={matchesSM ? "center" : undefined}>
            ...
            <Grid item container direction="column" alignItems={matchesSM ? "center" : undefined} md={5} style={{ maxWidth: "30em" }}>
              ...
              <Grid item>
                <Button variant="contained" className={classes.estimateButton} onClick={sendEstimate} disabled={name.length === 0 || message.length === 0 || phoneHelper.length !== 0 || emailHelper.length !== 0 || email.length === 0 || phone.length === 0}>
                  {loading ? (
                    <CircularProgress />
                  ) : (
                    <React.Fragment>
